...
namespace JasonHendry\TriagePHP;

class TriageClient {
    private static $triagezmqmsgpackclient;
    public static function error($url,$project,$errno, $errstr, $errfile, $errline, $errcontext)
    {
        if (self::$triagezmqmsgpackclient === null) {
            self::$triagezmqmsgpackclient = new TriageZeroMQMessagePackClient(['uri' => $url]);
        }
        $errorMap = [
            1 => 'Error',
            2 => 'Warning',
        ...
        $githash = is_file(dirname(__FILE__) . '/../.git/refs/heads/master')
            ...
            : (is_file(dirname(__FILE__) . '/commit.txt')
                ...
                : 'Unknown');
        self::$triagezmqmsgpackclient->logError([
            'message' => $errstr,
            'project' => $project,
            'type' => $errorType,
            'language' => 'php',
